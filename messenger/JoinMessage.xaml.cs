...
{
    /// <summary>
    /// Логика взаимодействия для JoinMessage.xaml
    /// </summary>
    public partial class JoinMessage : Window
    {
        ...
        public JoinMessage(string Name, string IP_Address)
        {
            InitializeComponent();
        ...
        private void Send_Click(object sender, RoutedEventArgs e)
        {
            ...
            switch (CommType)
            {
                case 1:
                    if (users_.Count == 0)
                    {
                        AllMessages.Items.Add("Здесь пусто");
                        return;
                    }
                    string info = $"[{DateTime.Now}] {name}: {Messagess.Text}";
                    AllMessages.Items.Add(info);
                ...
                case 2:
                    ...
                    {
                        AllMessages.Items.Add("Ошибка отправки, вы не подключены!");
                        return;
                    }
                    if (Messagess.Text == "/disconnect")
                        close();
                    else
                        Client.SendAsync(Encoding.UTF8.GetBytes(Messagess.Text), SocketFlags.None);
                    break;
            }
        }
        ...
        private async Task recv()
        {
            ...
            while (isConnected)
            {
                try
                {
                    ...
                    AllMessages.Items.Add($"{Encoding.UTF8.GetString(bt)}");
                }
                ...
                catch (SocketException)
                {
                    ...
                    {
                        AllMessages.Items.Add($"Клиент {users.Items[0]} Покинул сервер!");
                    ...
                    {
                        AllMessages.Items.Add("Сервер выключен");
                        users.Items.Clear();
                    }
                }
            }
        ...
        private async Task listen()
        {
            while (true)
            {
                ...
                AllMessages.Items.Add($"пользователь {users_[socket]} подключился!");
                users.ItemsSource = users_.Values.ToList();
                receive(socket);
            }
        }
        ...
        private async Task receive(Socket client_)
        {
            while (true)
            {
                ...
                {
                    AllMessages.Items.Add($"[{DateTime.Now}] Пользователь {users_[client_]} отлючился!");
                ...
                {
                    AllMessages.Items.Add($"[{DateTime.Now}]: пользователь {users_[client_]} отключился!");
                    users_.Remove(client_);
                    users.ItemsSource = users_.Values.ToList();
                    foreach (var sock in users_.Keys.ToList())
                    {
                ...
                else
                {
                    ...
                    AllMessages.Items.Add($"[{DateTime.Now}] {users_[client_]} : {msg}");
                }
                if (!IsConnected(client_))
                {
                    users_.Remove(client_);
